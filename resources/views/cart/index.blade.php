      ...
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div class="lg:col-span-2 bg-white p-8 rounded-2xl shadow-md border border-gray-100">
          @foreach ($cart as $id => $item)
            <div class="flex flex-col sm:flex-row sm:items-center justify-between border-b border-gray-200 pb-6 mb-6 last:border-none last:pb-0 last:mb-0 transition-all hover:scale-[1.01] hover:shadow-sm duration-200">
              ...
              <div class="mt-4 sm:mt-0 flex items-center gap-3">
                <input 
                  ...
                  value="{{ $item['quantity'] }}" 
                  min="1"
                  class="w-16 border rounded-lg text-center focus:ring-2 focus:ring-blue-500 focus:outline-none py-1 quantity-input shadow-sm"
                  data-id="{{ $id }}" 
                ...
                <form action="{{ route('cart.remove', $id) }}" method="POST" onsubmit="return confirm('Hapus produk ini?')">
                  @csrf
                  @method('DELETE')
                  <button type="submit" class="bg-red-600 text-white px-3 py-2 rounded-lg hover:bg-red-700 transition shadow-sm">
        ...
        <div class="bg-white p-8 rounded-2xl shadow-md border border-gray-100 h-fit sticky top-28">
          ...
          <div class="space-y-3 text-gray-700 mb-6">
            ...
            <div class="flex justify-between text-lg font-semibold border-t pt-3">
              <span>Total</span>
              <span class="text-blue-700" id="grand-total">Rp {{ number_format($grandTotal, 0, ',', '.') }}</span>
            </div>
          </div>
          ...
          <div class="space-y-3">
            <a href="{{ route('cart.checkout') }}" 
              class="block text-center bg-green-600 text-white py-3 rounded-xl hover:bg-green-700 transition font-semibold shadow-md hover:shadow-lg">
            ...
            <form action="{{ route('cart.remove', 'all') }}" method="POST" onsubmit="return confirm('Kosongkan keranjang?')">
              @csrf
              @method('DELETE')
              <button type="submit" class="w-full bg-gray-200 text-gray-700 py-3 rounded-xl hover:bg-gray-300 transition font-semibold shadow-sm hover:shadow">
